...
from extractors.custom_extractors.wamderivati.derivati_extractor import DerivatiKidExtractor
...
# TEMPORARY
from extractors.general_extractors.config.json_config import renaming
...
class BNPDerivatiKidExtractor(DerivatiKidExtractor):
    ...
    def extract_general_data(self):
        ...
        try:
            ...
            general_data_prompt = self.extraction_config["prompt"]["general_info"]
            general_data_schema = self.extraction_config["tag"]["general_info"]
            extraction.update(
                dict(llm_extraction_and_tag([self.text[1]], general_data_prompt, general_data_schema, self.file_id))
            )
    ...
    def extract_sottostanti(self):
        ...
        sottostante_bnp_schema = self.extraction_config["tag"]["sottostante_bnp"]
        extraction = general_table_inspection(sottostante, sottostante_bnp_schema, self.file_id)
        ...
        if len(self.di_tables_pages) > 3 and (
            ...
            extraction = general_table_inspection(sottostante, sottostante_bnp_schema, self.file_id, language=self.language)
        # extraction = clean_response_regex( "main_info", self.language, extraction)
    ...
    def extract_first_info(self):
        ...
        extraction = dict()
        try:
            first_info_prompt = self.extraction_config["prompt"]["first_info_bnp"]
            first_info_schema = self.extraction_config["tag"]["first_info_bnp"]
            extraction = llm_extraction_and_tag([self.text[0]], first_info_prompt, first_info_schema, self.file_id)
    ...
    def process(self):
        ...
        try:
            ...
            formatted_response = self.create_output(complete_response)
